...
def usage(fp = sys.stderr):
    ...
    msg += "\n"                                        \
           ...
           "  --name2 STR      (Optional) Name of app2: cellsnp-lite or freebayes.\n"   \
           "  --infile2 FILE   (Optional) Input tsv file of app2\n"          \
           "  --outfig FILE    Output figure\n"              \
           "  --title STR      Title of figure\n"
    msg += "  --color STR      color of dot/line [%s]\n" % COLOR
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "h", ["name1=", "infile1=", "name2=", 
             "infile2=", "outfig=", "title=", "color=", "width=", "height=", "dpi=", "help"])
    except getopt.GetoptError as e:
        print(str(e))
        usage()
        sys.exit(2)

    name1 = infile1 = name2 = infile2 = outfig = title = color = width = height = dpi = None
    ...
    if name2 is None or infile2 is None: 
        PR_plot(name1, infile1, outfig, title, color, width, height, dpi)
    else:
