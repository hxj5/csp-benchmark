...
run=1a_demuxlet
csp_dir=$RES_DIR/$run/run/cellsnp-lite_1_8
...
$BIN_DIR/Rscript $util_dir/mtx_get_ref.r \
  --ad $csp_dir/cellSNP.tag.AD.mtx \
  --dp $csp_dir/cellSNP.tag.DP.mtx \
  -o $csp_ref
...
$util_dir/accuracy.sh \
  --name1 cellsnp-lite     \
  --ref1 $csp_ref          \
  --alt1 $csp_dir/cellSNP.tag.AD.mtx   \
  --variant1 $csp_dir/cellSNP.base.vcf.gz  \
  --name2 vartrix           \
