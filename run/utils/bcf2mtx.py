#!/usr/bin/env python
#-*-coding:utf-8-*-
#Aim: to convert AD values of bcftools mpileup to cellSNP-style ref & alt matrices
...
def bcf2mtx(vcff0, vcff1, out_dir, prefix):
    """
        ...
        for smp_i, smp in enumerate(rec.samples.values()):
            ...
            if alt_idx >= 0:
                ...
                if valt > 0:
                    ...
                    nr_alt += 1
                    write_any = 1
            oth_allele_idx = [i for i, a in enumerate(rec.alleles) \
                    if a in ("A", "C", "G", "T", "N") and a != ref and a != alt]
            if oth_allele_idx:
...
if __name__ == "__main__":    
    ...
    ret = bcf2mtx(vcf0, vcf1, out_dir, prefix)
    if ret < 0:
        sys.stderr.write("Error: bcf2mtx error\n")
        sys.exit(ret)
